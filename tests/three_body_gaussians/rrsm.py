...
def reasonable_random_structure_maker(elements, composition_generator,
        cut_off_radius = 5.0, fill_factor_max = 0.55, fill_factor_min = 0.2,
        insert_attempt_max = 2000, max_build_failures = 20, element_radii =
        default_element_radii, hard_radii = default_hard_radii, magmom_generator
        = None, verbose = False):
...
if __name__ == "__main__":
    ...
    structure_dir = 'random_structures'
    if not os.path.exists(structure_dir):
        os.makedirs(structure_dir)

    def generate_random_si(elements, no_gas = True):
        import numpy as np
        if len(elements) == 1:
            return np.array([1])

        atomic_fractions = np.random.rand(len(elements))
    ...
    for structure_number in range(n_structures):
            ...
            atoms = reasonable_random_structure_maker(elements,
                        cut_off_radius = 3,
                        fill_factor_max = 0.6, # 0.65 is about the max
                        fill_factor_min = 0.2, #0.2 is default
                        composition_generator = generate_random_si)
            ...
            if True:
                    from ase import io
                    io.write('{}/{:03d}.vasp'.format(structure_dir,
                        structure_number), atoms, format = 'vasp', vasp5=True)
